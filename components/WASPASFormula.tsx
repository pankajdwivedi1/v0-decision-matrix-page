...
export default function WASPASFormula() {
  const latex = {
    decision_matrix: "X = [x_{ij}]_{m \\times n} = \\begin{bmatrix} x_{11} & x_{12} & \\dots & x_{1n} \\\\ x_{21} & x_{22} & \\dots & x_{2n} \\\\ \\vdots & \\vdots & \\ddots & \\vdots \\\\ x_{m1} & x_{m2} & \\dots & x_{mn} \\end{bmatrix}",
    step1_benefit: "\\overline{x}_{ij} = \\frac{x_{ij}}{\\max_i x_{ij}}",
    step1_cost: "\\overline{x}_{ij} = \\frac{\\min_i x_{ij}}{x_{ij}}",
    step2_wsm: "Q_i^{(1)} = \\sum_{j=1}^{n} \\overline{x}_{ij} \\cdot w_j",
    step3_wpm: "Q_i^{(2)} = \\prod_{j=1}^{n} (\\overline{x}_{ij})^{w_j}",
    step4_formula: "Q_i = \\lambda Q_i^{(1)} + (1 - \\lambda) Q_i^{(2)}, \\quad \\lambda \\in [0,1]",
    ...
    <>
      ...
      <div style={{ overflowWrap: "break-word", wordBreak: "break-word" }} className="prose max-w-none bg-white border border-gray-200 rounded-lg p-3 md:p-6 text-justify font-['Times_New_Roman',_Times,_serif] leading-relaxed">
        ...
        <div className="bg-gray-50 rounded-lg p-4 mb-4 overflow-x-auto">
          <p className="text-center">{`$$ ${latex.decision_matrix} $$`}</p>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-4 mb-4 overflow-x-auto">
          <p className="text-center">{`$$ ${latex.step2_wsm} $$`}</p>
        </div>
