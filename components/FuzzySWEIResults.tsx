...
interface FuzzySWEIResultsProps {
  fuzzyResult: FuzzySWEIResult;
  criteria: Criterion[];
...
export default function FuzzySWEIResults({
  fuzzyResult,
  criteria,
  alternatives,
    ...
    <div className="space-y-6">
      ...
      <Card className="border-gray-200 bg-white shadow-none">
        <div className="px-6 pt-4">
          <ResearchAssetHeader
            assetKey="fuzzy_swei_normalized_matrix"
            defaultLabel="Table R4"
            title="SWEI FUZZY NORMALIZED INFORMATION DECISION MATRIX (IDM)"
            included={selectedAiAssets.has("fuzzy_swei_normalized_matrix")}
      ...
      {/* Table R5: Information matrix */}
      <Card className="border-gray-200 bg-white shadow-none">
        <div className="px-6 pt-4">
          <ResearchAssetHeader
            assetKey="fuzzy_swei_information"
            defaultLabel="Table R5"
            title="SWEI FUZZY INFORMATION VALUES (log2(1/IDM))"
            included={selectedAiAssets.has("fuzzy_swei_information")}
            onIncludeChange={handleIncludeChange}
            onLabelChange={handleAssetLabelChange}
          />
        </div>
              ...
              <TableBody>
                {alternatives.map((alt) => (
                  <TableRow key={alt.id} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    {criteria.map((crit) => (
                      <TableCell key={crit.id} className="text-center py-3 px-4 text-xs text-black">
                        {formatFuzzy(fuzzyResult.fuzzyInformationMatrix[alt.id]?.[crit.id])}
                      </TableCell>
                    ))}
                  </TableRow>
                ))}
      ...
      {/* Table R6: Weighted Exponential matrix */}
      <Card className="border-gray-200 bg-white shadow-none">
        <div className="px-6 pt-4">
          <ResearchAssetHeader
            assetKey="fuzzy_swei_weighted_exponential"
            defaultLabel="Table R6"
            title="SWEI FUZZY WEIGHTED EXPONENTIAL INFORMATION"
            included={selectedAiAssets.has("fuzzy_swei_weighted_exponential")}
            onIncludeChange={handleIncludeChange}
            onLabelChange={handleAssetLabelChange}
          />
        </div>
              ...
              <TableBody>
                {alternatives.map((alt) => (
                  <TableRow key={alt.id} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    {criteria.map((crit) => (
                      <TableCell key={crit.id} className="text-center py-3 px-4 text-xs text-black">
                        {formatFuzzy(fuzzyResult.fuzzyWeightedExponentialMatrix[alt.id]?.[crit.id])}
                      </TableCell>
                    ))}
                  </TableRow>
                ))}
