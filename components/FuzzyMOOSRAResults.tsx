...
interface FuzzyMOOSRAResultsProps {
  fuzzyResult: FuzzyMOOSRAResult;
  criteria: Criterion[];
...
export default function FuzzyMOOSRAResults({
  fuzzyResult,
  criteria,
  alternatives,
    ...
    <div className="space-y-6">
      ...
      <Card className="border-gray-200 bg-white shadow-none">
        <div className="px-6 pt-4">
          <ResearchAssetHeader
            assetKey="fuzzy_moosra_normalized_matrix"
            defaultLabel="Table R4"
            title="FUZZY NORMALIZED DECISION MATRIX"
            included={selectedAiAssets.has("fuzzy_moosra_normalized_matrix")}
      ...
      {/* Table R5: Beneficial vs Non Beneficial Sums */}
      <Card className="border-gray-200 bg-white shadow-none">
        <div className="px-6 pt-4">
          <ResearchAssetHeader
            assetKey="fuzzy_moosra_sums"
            defaultLabel="Table R5"
            title="MOOSRA BENEFICIAL & NON-BENEFICIAL ASSESSMENTS"
            included={selectedAiAssets.has("fuzzy_moosra_sums")}
            onIncludeChange={handleIncludeChange}
            onLabelChange={handleAssetLabelChange}
          />
        </div>
          ...
          <div className="table-responsive border border-gray-200 rounded-lg overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50 border-b border-gray-200">
                  <TableHead className="text-xs font-semibold text-black py-3 px-4">Alternative</TableHead>
                  <TableHead className="text-xs font-semibold text-center py-3 px-4 text-green-600">Beneficial Sum (B)</TableHead>
                  <TableHead className="text-xs font-semibold text-center py-3 px-4 text-red-600">Non-Beneficial Sum (C)</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {alternatives.map((alt) => (
