...
export default function EntropyFormula({ compact = false }: EntropyFormulaProps) {
  ...
  const latex = {
    ...
    step2_benefit:
      "p_{ij} = \\frac{x_{ij}}{\\sum_{i=1}^{m} x_{ij}}, \\quad \\text{for benefit (max) criteria}",
    step2_cost:
      "p_{ij} = \\frac{1/x_{ij}}{\\sum_{i=1}^{m} (1/x_{ij})}, \\quad \\text{for cost (min) criteria}",
    step3_intro:
      "\\textbf{3. Entropy Calculation:} \\quad \\text{Calculate the entropy value for each criterion to measure the information content.}",
    step3_formula:
      "E_j = -k \\sum_{i=1}^{m} p_{ij} \\ln(p_{ij}), \\quad \\text{where } k = \\frac{1}{\\ln(m)} \\text{ and } j = 1, 2, \\ldots, n",
    step4_intro:
