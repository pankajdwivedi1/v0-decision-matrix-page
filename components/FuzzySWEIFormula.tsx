...
export default function FuzzySWEIFormula() {
  const latex = {
    ...
    step4: "\\tilde{v}_{i,j} = (\\tilde{I}_{i,j})^{w_j} = (l_{I_{i,j}}^{w_j}, m_{I_{i,j}}^{w_j}, u_{I_{i,j}}^{w_j}) \\tag{6}",
    step5: "S_i = \\sum_{j=1}^n \\tilde{v}_{i,j} \\tag{7}",
    step6: "F-SWEI_i'' = \\text{Defuzzify}(S_i) = \\frac{l_{S_i} + m_{S_i} + u_{S_i}}{3} \\tag{8}",
    ranking: "Rank(X_i) \\uparrow \\text{ as } F-SWEI_i'' \\downarrow \\text{ (Ascending)}"
  }
  ...
  return (
    <>
      ...
      <div ref={containerRef} className="prose max-w-none bg-white border border-gray-200 rounded-lg p-3 md:p-6 text-justify font-['Times_New_Roman',_Times,_serif] leading-relaxed">
        <h1 className="text-2xl font-bold text-center mb-6">Fuzzy SWEI Method</h1>
        <p className="mb-4">Fuzzy SWEI (Sum Weighted Exponential Information) calculates the information content of fuzzy alternatives using an exponential weighting approach.</p>

        <h2 className="text-xl font-semibold mt-6 mb-2">Step 1. Fuzzy Decision Matrix</h2>
        <p className="mb-2">The decision-maker constructs the fuzzy information decision matrix (IDM) in the first step, where each element is a TFN triplet (l, m, u):</p>
        ...
        <p className="mb-2">Calculate the information content (IDM) using base-2 logarithm:</p>
        <div className="bg-gray-50 rounded-lg mb-4">
          <div className="latex text-sm text-center" dangerouslySetInnerHTML={{ __html: `\\[${latex.step3}\\]` }} />
        </div>

        <h2 className="text-xl font-semibold mt-6 mb-2">Step 4. Weighted Exponential Matrix</h2>
        <p className="mb-2">Raise the information content to the power of the criterion weight:</p>
        <div className="bg-gray-50 rounded-lg mb-4">
        ...
        <p className="mb-2">Sum the weighted exponential information values for each alternative:</p>
        <div className="bg-gray-50 rounded-lg mb-4">
        ...
        <p className="mb-2">Calculate the final crisp score and rank alternatives (typically smaller is better in information theory):</p>
        <div className="bg-gray-50 rounded-lg mb-4">
