...
export default function VIKORFormula({ compact = false }: VIKORFormulaProps) {
  ...
  const latex = {
    ...
    step2_benefit:
      "f_{ij} = \\frac{x_{ij}}{\\max_i x_{ij}}, \\quad \\text{for benefit (max) criteria}",
    step2_cost:
      "f_{ij} = \\frac{\\min_i x_{ij}}{x_{ij}}, \\quad \\text{for cost (min) criteria}",
    step3_intro:
      "\\textbf{3. Best and Worst Values:} \\quad \\text{Determine the best } f_j^* \\text{ and worst } f_j^- \\text{ values for each criterion.}",
    step3_best:
      "f_j^* = \\begin{cases} \\max_i f_{ij} & \\text{if } j \\in \\text{beneficial} \\\\ \\min_i f_{ij} & \\text{if } j \\in \\text{non-beneficial} \\end{cases}",
    step3_worst:
      "f_j^- = \\begin{cases} \\min_i f_{ij} & \\text{if } j \\in \\text{beneficial} \\\\ \\max_i f_{ij} & \\text{if } j \\in \\text{non-beneficial} \\end{cases}",
    step4_intro:
      "\\textbf{4. Utility Measure (S_i):} \\quad \\text{Calculate the weighted sum of normalized distances for each alternative.}",
    step4_formula:
      "S_i = \\sum_{j=1}^{n} w_j \\frac{f_j^* - f_{ij}}{f_j^* - f_j^-}, \\quad \\text{where } \\sum_{j=1}^{n} w_j = 1",
    step5_intro:
