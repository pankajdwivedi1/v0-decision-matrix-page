...
import { FuzzyTOPSISResult, Criterion, Alternative, FuzzyNumber } from "@/types/mcdm";
import { Card, CardContent } from "@/components/ui/card";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { ResearchAssetHeader } from "./ResearchAssetHeader";

interface FuzzyTOPSISResultsProps {
  fuzzyResult: FuzzyTOPSISResult;
  criteria: Criterion[];
  ...
  fpisLabel: string;
  weightedMatrixLabel: string;
}

export default function FuzzyTOPSISResults({
  fuzzyResult,
  criteria,
  alternatives,
    ...
    <div className="space-y-6">
      ...
      <Card className="border-gray-200 bg-white shadow-none">
        <div className="px-6 pt-4">
          <ResearchAssetHeader
            assetKey="fuzzy_topsis_normalized_matrix"
            defaultLabel="Table R4"
            title="FUZZY NORMALIZED DECISION MATRIX"
            included={selectedAiAssets.has("fuzzy_topsis_normalized_matrix")}
      ...
      <Card className="border-gray-200 bg-white shadow-none">
        <div className="px-6 pt-4">
          <ResearchAssetHeader
            assetKey="fuzzy_topsis_weighted_matrix"
            defaultLabel="Table R5"
            title="FUZZY WEIGHTED NORMALIZED MATRIX"
            included={selectedAiAssets.has("fuzzy_topsis_weighted_matrix")}
            onIncludeChange={handleIncludeChange}
            onLabelChange={handleAssetLabelChange}
          />
        </div>
      ...
      {/* Table R7: Distances and Closeness Coefficient */}
      <Card className="border-gray-200 bg-white shadow-none">
        <div className="px-6 pt-4">
          <ResearchAssetHeader
            assetKey="fuzzy_topsis_distances"
            defaultLabel="Table R7"
            title="DISTANCES AND CLOSENESS COEFFICIENTS"
            included={selectedAiAssets.has("fuzzy_topsis_distances")}
            onIncludeChange={handleIncludeChange}
            onLabelChange={handleAssetLabelChange}
          />
        </div>
          ...
          <div className="table-responsive border border-gray-200 rounded-lg overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50 border-b border-gray-200">
                  <TableHead className="text-xs font-semibold text-black py-3 px-4">Alternative</TableHead>
                  <TableHead className="text-xs font-semibold text-center py-3 px-4">Distance to FPIS (d*)</TableHead>
                  <TableHead className="text-xs font-semibold text-center py-3 px-4">Distance to FNIS (d-)</TableHead>
                  <TableHead className="text-xs font-semibold text-center py-3 px-4">Closeness Coeff (CC)</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {alternatives.map((alt) => (
                  <TableRow key={alt.id} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <TableCell className="text-center py-3 px-4 text-xs text-black">
                      {fuzzyResult.distances[alt.id]?.positive.toFixed(resultsDecimalPlaces + 2)}
                    </TableCell>
                    <TableCell className="text-center py-3 px-4 text-xs text-black">
                      {fuzzyResult.distances[alt.id]?.negative.toFixed(resultsDecimalPlaces + 2)}
                    </TableCell>
                    <TableCell className="text-center py-3 px-4 text-xs font-bold text-blue-700">
                      {fuzzyResult.scores[alt.id]?.toFixed(resultsDecimalPlaces + 4)}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
