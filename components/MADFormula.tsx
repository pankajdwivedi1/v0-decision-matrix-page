...
export default function MADFormula() {
    const latex = {
        step1: "n_{ij} = \\frac{x_{ij}}{\\sum_{k=1}^{m} x_{kj}} \\tag{1}",
        step2: "MAD_j = \\frac{1}{m} \\sum_{i=1}^{m} |n_{ij} - \\bar{n}_j| \\tag{2}",
        step3: "w_j = \\frac{MAD_j}{\\sum_{k=1}^{n} MAD_k} \\tag{3}"
    }
    ...
    return (
        ...
        `
            ...
            <div ref={containerRef} className="prose max-w-none bg-white border border-gray-200 rounded-lg p-3 md:p-6 text-justify font-['Times_New_Roman',_Times,_serif] leading-relaxed">
                <h1 className="text-2xl font-bold text-center mb-6">Mean Absolute Deviation (MAD) Weight Method</h1>
                <p className="mb-4">
                    The Mean Absolute Deviation (MAD) method is an <strong>objective</strong> weighting technique that measures dispersion using absolute differences from the mean, making it less sensitive to outliers than variance.
                </p>
                ...
                <h2 className="text-xl font-semibold mt-6 mb-2">Step 2. MAD Calculation</h2>
                <p className="mb-2">Calculate the Mean Absolute Deviation for each criterion:</p>
                <div className="bg-gray-50 rounded-lg p-4 mb-4">
                ...
                <p className="mb-2">Weights are proportional to the calculated MAD values:</p>
                <div className="bg-gray-50 rounded-lg p-4 mb-4">
