...
# 1. Identify the block from chartElement start to SVG end
START = -1
END   = -1
for i in range(7380, 7450):
    if 'chartElement = (' in lines[i]:
        START = i
        break

if START == -1:
    print("Could not find chartElement start.")
    exit(1)

# Find where the SVG ends and the tooltip ends
for i in range(START, len(lines)):
    if ');' in lines[i] and 'break;' in lines[i+1]:
        END = i
        break

if END == -1:
    print("Could not find chartElement end.")
    exit(1)

NEW_BODY = r'''                                    const isHorizontal = chartSettings.legendLayout === 'horizontal';
