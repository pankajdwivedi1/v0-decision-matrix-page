...
export function calculateFuzzyMOORA(
  ...
  try {
    ...
    const overallAssessment: Record<string, FuzzyNumber> = {};
    ...
    for (let i = 0; i < m; i++) {
      ...
      const overall_l = ben_l - cost_u;
      const overall_m = ben_m - cost_m;
      const overall_u = ben_u - cost_l;

      overallAssessment[alternatives[i].id] = { l: overall_l, m: overall_m, u: overall_u };

      scores[alternatives[i].id] = (overall_l + overall_m + overall_u) / 3;
    }
    ...
    console.error("CRASH IN FUZZY MOORA:", err);
    throw err;
  }
}
